	...
	private function MissionCaseMIP ($FleetRow)
	{
		...
		if ($FleetRow['fleet_start_time'] <= time())
		{
			if ($FleetRow['fleet_mess'] == 0)
			{
				...
				$Target = doquery('SELECT id, defence_tech FROM  {{table}} WHERE `galaxy` = '.$FleetRow['fleet_end_galaxy'].' AND  `system` = '.$FleetRow['fleet_end_system'].' AND `planet` =  '.$FleetRow['fleet_end_planet'], 'planets', TRUE);
